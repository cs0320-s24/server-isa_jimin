...
public class TestSoupAPIHandlers {
  ...
  // Recall that the "throws IOException" doesn't signify anything but acknowledgement to the type
  // checker
  public void testAPIPotatoRecipe() throws IOException {
    menu.add(
        Soup.buildNoExceptions(
            "Potato",
            Arrays.asList("potato", "onion", "cheese", "garlic", "bacon", "vegetable broth")));

    HttpURLConnection clientConnection = tryRequest("order", "Potato");
    // Get an OK response (the *connection* worked, the *API* provides an error response)
    assertEquals(200, clientConnection.getResponseCode());
    ...
    Soup potato =
        new Soup(
            ...
            Arrays.asList("potato", "onion", "cheese", "garlic", "bacon", "vegetable broth"),
            true);
    Map<String, Object> result = (Map<String, Object>) response.responseMap().get("Potato");
    System.out.println(result.get("ingredients"));
    assertEquals(potato.getIngredients(), result.get("ingredients"));
    clientConnection.disconnect();
  }
}
