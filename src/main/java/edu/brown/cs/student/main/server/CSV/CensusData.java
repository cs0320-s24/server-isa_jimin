...
public class CensusData {
    ...
    public List<List<String>> searchColName(String query, String colName) throws IOException, FactoryFailureException {
        Search search = new Search(this.parser, query);
        return search.colSearch(colName, null);
    }

    public List<List<String>> searchColIndex(String query, Integer index) throws IOException, FactoryFailureException {
        Search search = new Search(this.parser, query);
        return search.colSearch(null, index);
    }

    public List<List<String>> searchNoHeader(String query) throws IOException, FactoryFailureException {
        Search search = new Search(this.parser, query);
    ...
    /**
     * This is the Create class which we chose to nest in DataHandler so data could utilize a create method
     * without having to implement CreatorFromRow since it is not a create object.
     */
